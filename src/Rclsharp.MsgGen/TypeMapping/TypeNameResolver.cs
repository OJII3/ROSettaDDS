...
namespace Rclsharp.MsgGen.TypeMapping;

/// <summary>
/// ROS パッケージ/型名から C# の名前空間・型名・シリアライザ名・DDS 型名を決める。
///
/// 既存の手書き <c>Rclsharp.Msgs.*</c> と一致させるためのオーバライドを持つ:
/// <list type="bullet">
/// <item>BCL と衝突する std_msgs のスカラ型は <c>Message</c> サフィックス付き
/// (案 B / .claude/CLAUDE.md)。</item>
/// <item>名前空間は <c>std_msgs → Rclsharp.Msgs.Std</c> のように <c>_msgs</c> を畳む。</item>
/// </list>
/// </summary>
public sealed class TypeNameResolver
{
    public const string RootNamespace = "Rclsharp.Msgs";
    ...
    /// <summary>完全な名前空間 (例 "Rclsharp.Msgs.Std")。</summary>
    public string Namespace(string package) => $"{RootNamespace}.{SubNamespace(package)}";

    /// <summary>名前空間付きの C# 型名 (例 "Rclsharp.Msgs.Std.Header")。</summary>
