...
- Android 上のメモリリーク guard (IL2CPP の GC 挙動は Mono と違うので別途
  検討)。
- ユニキャスト discovery 経路。**rosettadds は独自 DDS 実装 (rmw_fastrtps_cpp /
  Fast DDS を経由しない) なため、`ROS_STATIC_PEERS` env 注入しても無視される**。
  真の unicast discovery サポートは rosettadds 自体に SPDP writer の
  destination に static peer locator を追加する必要があり、別 PR のスコープ。
  当面は host と Android 端末を同一 L2 segment に置いて SPDP multicast
  (239.255.0.1) を通す前提を維持する。
