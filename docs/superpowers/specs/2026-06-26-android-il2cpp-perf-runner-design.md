...
## ユニキャスト discovery (subnet 越え対応)

host と Android 端末が異なる subnet にいる場合、SPDP multicast (239.255.0.1:7400)
が届かない。Phase 2b で `ROS_STATIC_PEERS` env var 注入による static peer
discovery 経路を追加した。

- RunnerOptions: `--rosettadds-static-peer <ip>` (optional)。指定されると:
  - helper の env に `ROS_STATIC_PEERS=<ip>:7411` が追加される (helper は `<ip>`
    を静的 peer として認識)
  - player の CLI args に `--rosettadds-static-peer <ip>` が追加される
  - player の `PerfPlayerEntry` は起動時に
    `Environment.SetEnvironmentVariable("ROS_STATIC_PEERS", "<ip>:7411")` を呼んで
    env に注入
- `--rosettadds-host-ip <ip>`: host IP を明示 (将来 player 側で host の IP を
  知る必要がある場合の hook、現状未使用)
...
- ユニキャスト discovery 経路 (`ROS_STATIC_PEERS` 等)。マルチキャストが
  通らないネットワーク形態。`ROS_STATIC_PEERS` 経由の static peer discovery は
  Phase 2b で実装済み。
